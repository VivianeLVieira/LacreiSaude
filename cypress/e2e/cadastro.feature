...
Feature: Cadastro da pessoa usuária no sistema
  ...
  Scenario Outline: Usuário realiza cadastro
    ...
    Then ele deve ver <resultado> #bugs #atualmente existe um redirecionamento da tela (que informa ao usuário que ele precisa confirmar a criação da conta via email) para a tela de login, que impossibilita validar a mensagem de cadastro, inclusive o usuário também não consegue lê-la
    #Acessar link API para confirmar email automaticamente sem precisar abrir o email manualmente #melhoria
    #continuar teste na tela de pós-cadastro pra concluir todo processo de cadastro #bug
